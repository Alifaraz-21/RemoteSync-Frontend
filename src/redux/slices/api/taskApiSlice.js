...
export const postApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    uploadImage: builder.mutation({
      query: ({ taskId, uint8Array }) => ({
        url: `${TASKS_URL}/upload_screenshot/${taskId}`,
        method: "PUT",
        body: uint8Array, // Send raw binary data directly
        headers: {
    ...
    getTaskScreenshots: builder.query({
  query: (taskId) => ({
    url: `${TASKS_URL}/screenshots/${taskId}`, // Fetch screenshots for a specific task
    method: "GET",
