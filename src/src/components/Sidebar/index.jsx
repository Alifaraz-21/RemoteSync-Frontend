...
const Sidebar = () => {
  ...
  return (
    <div className="fixed left-0 top-0 md:w-[230px] w-[60px] overflow-hidden h-full flex flex-col z-50">
      <div className="w-full flex items-center md:justify-start justify-center md:pl-5 h-[70px] bg-[#fff]">
        <span className="text-[#786FB9] font-semibold text-[27px] md:block hidden hover:scale-110 cursor-pointer duration-300 ease-in-out">
          RemoteSync
  ...
  {navLinks.map((link) => {
    return (
      <div
        key={link.title}
        className={`flex items-center gap-2 w-full rounded-lg hover:bg-[#786FB9] hover:text-white px-2 py-3 cursor-pointer duration-300 ease-in-out group ${
          link.active ? "bg-[#786FB9] text-white" : "bg-transparent"
        }`}
      >
        ...
        <span className="font-medium text-[15px] md:block hidden group-hover:text-white">
          {link.title}
        </span>
      </div>
    );
